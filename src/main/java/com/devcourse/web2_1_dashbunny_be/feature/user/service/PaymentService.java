...
public class PaymentService {
    ...
    public PaymentResponseDto createPayment(PaymentRequestDto requestDto) {
        ...
        Cart cart = cartRepository.findById(requestDto.getCartId())
                .orElseThrow(() -> {
                    logger.error("Cart with id {} not found.", requestDto.getCartId());
        ...
        StoreManagement store = storeManagementRepository.findByStoreId(cart.getStoreId())
                .orElseThrow(() -> {
                    logger.error("Store with id {} not found.", cart.getStoreId());
        ...
        paymentRepository.save(payment); // Persist to get 'id'
        ...
        // `redirectUrl`은 제거되었습니다.

        HttpEntity<String> entity = new HttpEntity<>(body.toString(), headers);

        try {
            ResponseEntity<String> response = restTemplate.exchange(url, HttpMethod.POST, entity, String.class);
            logger.info("Toss Payments API response status: {}", response.getStatusCode());
            logger.info("Toss Payments API response body: {}", response.getBody());
